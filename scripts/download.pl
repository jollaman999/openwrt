...
sub download_cmd($) {
	...
	return $have_curl
		? (qw(curl -f --connect-timeout 5 --retry 3 --location --insecure), shellwords($ENV{CURL_OPTIONS} || ''), $url)
		: (qw(wget --tries=3 --timeout=5 --no-check-certificate --output-document=-), shellwords($ENV{WGET_OPTIONS} || ''), $url)
	;
}
